...
myTableGrob <- function(data_dt, title_v, fontsize_v = 14){
  ...
  ## Table
  table_grob <- gridExtra::tableGrob(data_dt, rows = rep('', nrow(data_dt)), theme = gridExtra::ttheme_minimal())
  ## Title
  title_grob <- grid::textGrob(title_v, gp = grid::gpar(fontsize = fontsize_v))
  ## Add title
