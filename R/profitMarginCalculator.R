profitMarginCalculator <- function(price_v, margin_v = NULL, shippingWeight_v = 0, recurring_v = 1, shop_v,
                                   includeShippingInMargin_v = F, method_v = 1, factor_v = 2, itemNum_v = 1) {
  #' Profit Margin Calculator
  #' @description Calculate appropriate price depending on desired margin and potential shipping costs.
  ...
  #' @param recurring_v multiplier for recurring shipping charges.
  ...
  #' @param itemNum_v number of items ordered (passed to shippingCost)
  #' @details
  ...
  ### Get shipping price
  shippingCost_v <- wrh.rUtils::shippingCalculator(weight_v = shippingWeight_v, shop_v = shop_v, itemNum_v = itemNum_v)
  
  ### Factor in recurring shipping
  shippingCost_v <- shippingCost_v * recurring_v
  ...
  cat(sprintf(paste0("\tWholesale price: %s\n
    ...
    paste_v,
    "\tMargin:%s\n
    Sale Price (price + shipping + markup): %s\n"), price_v, shippingWeight_v, shippingCost_v, markup_v, margin_v, finalPrice_v))
